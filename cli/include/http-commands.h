...
inline void http_payload_getbyrun( const std::vector<std::string>& args ) {
  ...
  if ( !res.size() ) {
    ...
    for( const auto& [ key, p ] : res ) {
      std::cout << "got payload: " << p << std::endl;
    }
  }
}

inline void http_payload_getbytime( const std::vector<std::string>& args ) {
  if ( args.size() < 3 ) {
...
inline void http_tags_list( __attribute__ ((unused)) const std::vector<std::string>& args ) {
	Service db;
	db.init("http");
  std::shared_ptr<PayloadAdapterHttp> adapter = std::dynamic_pointer_cast<PayloadAdapterHttp>( db.getPayloadAdapterHttp() );
...
inline void http_tags_create( const std::vector<std::string>& args ) {
  if ( args.size() < 2 ) {
...
inline void http_tags_deactivate( const std::vector<std::string>& args ) {
  if ( args.size() < 3 ) {
  ...
  if ( res.valid() ) {
    ...
    std::cout << "failed to create tag: " << args[1] << ", dt: " << dt << ", msg: " << res.msg() << "\n";
  }
}

inline void http_struct_create( const std::vector<std::string>& args ) {
  if ( args.size() < 3 ) {
...
inline void http_schema_set( const std::vector<std::string>& args ) {
  if ( args.size() < 3 ) {
    std::cerr << "please provide <tag-path>/<structName> <file> as arguments" << std::endl;
    return;
  }
  ...
  Service db;
  db.init("http");
  Result<bool> res = db.setTagSchema( args[1], schema_json );
  if ( res.valid() ) {
...
inline void http_schema_get( const std::vector<std::string>& args ) {
  if ( args.size() < 2 ) {
...
inline void http_schema_drop( const std::vector<std::string>& args ) {
  if ( args.size() < 2 ) {
    std::cerr << "please provide <tag-path>/<structName> as an argument" << std::endl;
    return;
  }
...
inline void http_tables_create( __attribute__ ((unused)) const std::vector<std::string>& args ) {
  Service db;
  db.init("http");
  std::shared_ptr<PayloadAdapterHttp> adapter = std::dynamic_pointer_cast<PayloadAdapterHttp>( db.getPayloadAdapterHttp() );
  Result<bool> rc = adapter->createDatabaseTables();
  if ( rc.valid() ) {
    std::cout << "database is initialized" << std::endl;
  } else {
    std::cerr << "failed to create database tables, " << rc.msg() << std::endl;
  }
}

inline void http_tables_drop( __attribute__ ((unused)) const std::vector<std::string>& args ) {
  Service db;
  db.init("http");
  std::shared_ptr<PayloadAdapterHttp> adapter = std::dynamic_pointer_cast<PayloadAdapterHttp>( db.getPayloadAdapterHttp() );
...
inline void http_tables_list( __attribute__ ((unused)) const std::vector<std::string>& args ) {
  Service db;
  db.init("http");
  std::shared_ptr<PayloadAdapterHttp> adapter = std::dynamic_pointer_cast<PayloadAdapterHttp>( db.getPayloadAdapterHttp() );
  std::vector<std::string> tables = adapter->listDatabaseTables();
  if ( tables.size() ) {
    std::cout << tables.size() << " [http] database tables: " << "\n";
...
inline void http_tags_export( const std::vector<std::string>& args ) {
  if ( args.size() < 2 ) {
...
inline void http_tags_import( const std::vector<std::string>& args ) {
  if ( args.size() < 2 ) {
